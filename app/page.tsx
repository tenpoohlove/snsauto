  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({ prompt: aiPrompt || caption.slice(0, 100) }),
      })
      const data = await res.json()
      if (res.ok) setImageUrl(data.url)
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-1">
        {([['url', 'URL入力'], ['upload', 'アップロード'], ['ai', 'AI生成']] as [ImageMode, string][]).map(([v, l]) => (
          <button
            key={v}
            onClick={() => setMode(v)}
            className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
              mode === v ? 'bg-blue-500 text-white border-blue-500' : 'bg-white text-gray-500 border-gray-200 hover:border-gray-400'
            }`}
          >
            {l}
          </button>
      ...
      {mode === 'url' && (
        <input
          ...
          className="w-full px-3 py-2 border rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
        />
      )}
      ...
      {mode === 'upload' && (
        <div>
          ...
          <button
            ...
            disabled={uploading}
            className="w-full py-2 border-2 border-dashed border-gray-300 rounded-lg text-xs text-gray-500 hover:border-blue-400 hover:text-blue-500 disabled:opacity-50"
          >
            {uploading ? 'アップロード中...' : 'クリックして画像を選択（JPG/PNG/WEBP 10MB以内）'}
          </button>
          {imageUrl && <p className="text-xs text-green-600 mt-1">アップロード完了</p>}
        </div>
      )}
      ...
      {mode === 'ai' && (
        <div className="space-y-2">
          <input
            type="text"
            placeholder="画像の内容を入力（省略時は投稿文から自動生成）"
            value={aiPrompt}
          ...
          <button
            ...
            disabled={generating}
            className="px-4 py-1.5 bg-purple-500 text-white rounded-lg text-xs font-medium hover:bg-purple-600 disabled:opacity-50"
          >
            {generating ? 'AI生成中...' : 'DALL-E 3で画像を生成'}
          </button>
          {imageUrl && (
            <img src={imageUrl} alt="生成画像" className="w-32 h-32 object-cover rounded-lg border" />
          )}
        </div>
      )}
    ...
    <div className="flex flex-col h-screen bg-gray-50">
      <header className="flex items-center justify-between px-6 py-4 bg-white border-b">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/settings" className="text-sm text-gray-500 hover:text-gray-800">設定</Link>
          <UserButton />
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4 max-w-3xl mx-auto w-full">
          ...
          if (m.role === 'user') {
            return (
              <div key={m.id} className="flex justify-end">
                <div className="max-w-[80%] px-4 py-3 rounded-2xl text-sm bg-blue-500 text-white">{text}</div>
              </div>
            )
          }
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-white border shadow-sm px-4 py-3 rounded-2xl text-sm text-gray-400">生成中...</div>
          </div>
        )}
      </div>
